...
<div id="urHere">DouPHP 管理中心<b>></b><strong></strong>用户管理</div>   <div id="manager" class="mainBox" style="height:auto!important;height:550px;min-height:550px;">
    <h3>
        {{--<a href="addmanager.html?rec=add" class="actionBtn">添加管理员</a>--}}
        用户管理
    </h3>
     ...
     <tr>
      ...
      <td>{{$u->user_name}}</td>
      <td align="center">{{$u->user_password}}</td>
      <td align="center">{{$u->user_nickname}}</td>
      <td align="center">{{$u->user_phone}}</td>
      <td align="center">{{$u->user_email}}</td>
      <td align="center"><span onclick="sp({{$u->user_id}})" id="s_{{$u->user_id}}">{{$u->user_score}}</span>
      <input type="text" value="{{$u->user_score}}" style="display:none;width:50px" id="i_{{$u->user_id}}"  onblur="ob({{$u->user_id}})"/>
      </td>
